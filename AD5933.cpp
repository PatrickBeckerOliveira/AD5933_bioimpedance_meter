 ...
 * @return Success or failure
 */
bool AD5933::calibrate(double gain[], int phase[], long int ref, int n) {
    ...
    for (int i = 0; i < n; i++) {
        gain[i] = (double)(ref)/sqrt(pow(real[i], 2) + pow(imag[i], 2)); // original: gain[i] = (double)(1.0/ref)/sqrt(pow(real[i], 2) + pow(imag[i], 2));
        phase[i] = (180 / M_PI) * atan((double)imag[i] / (double)real[i]);
        if (real[i] < 0)
            phase[i] = phase[i] + 180;
    }
